...
describe("Minting the token and returning it", function () {
  it("should the contract be able to mint a function and return it", async function () {
    ...
    const LegalContract = await ethers.getContractFactory("legalNFT"); // Getting the contract

    const legalContract = await LegalContract.deploy(); //Deploying the Contract

    const transaction = await legalContract.mintToken(metadata); // Minting the token
    ...
    const tokenURI = await legalContract.tokenURI(tokenId); // Using the tokenURI from ERC721 to retrieve de metadata
